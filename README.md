...
- **Готовое приложение:** папка `Applications for configuration on X86`
- **Исходные файлы:** папка `The source data of the application`
...
![Схема подключения](data/images//scheme.png)

*Настройки пинов в файле User_Setup.h библиотеки TFT_eSPI (уже настроены согласно схеме)*

### 3. Настройка Telegram бота

Подробная инструкция визуально представлена в приложении во вкладке "Инструкция получения токена бота..."
...
2. Откройте файл `main.cpp` в Arduino IDE

3. Настройте параметры по умолчанию в структуре `startData` (необязательно, так как в приложении для Windows можно будет настроить):
```cpp
struct Data {
...
Откройте уже собранное приложение из папки `Applications for configuration on X86`

### 6. Настройка устройства через приложение

1. Подключите ESP32 к компьютеру через USB
2. Запустите приложение настройки
3. При загрузке ESP32 в течение 15 секунд нажмите "Подключить устройство"
4. В приложении нажмите "Настройка параметров устройства"
5. Введите данные Wi-Fi сети и Telegram
...
1. На экране появится сообщение о необходимости настройки
2. Подключите ESP32 к ПК через USB
3. В течение 15 секунд отправьте символ `'` через COM-порт
4. Устройство войдет в режим настройки
5. Используйте Qt-приложение для изменения параметров
...
При включении на экране высвечивается логотип университета:

![Запуск устройства](data/images//start.png)

### Подключение к сети

После включения устройство автоматически попытается подключиться к сети интернет по стандартным параметрам:

![Подключение к сети](data/images//connect.png)

### Ошибка подключения

Если подключение будет неудачным, то устройство предложит изменить параметры подключения через персональный компьютер:

![Ошибка подключения](data/images//err_connect.png)

*На подключение к компьютеру выделяется 15 секунд, иначе устройство автоматически перезагрузится и повторит попытку подключиться к интернету с прежними настройками.*

### Настройка через Qt-приложение

Для введения устройства в режим «изменения настроек» следует подключить устройство к компьютеру. Для этого нужно выбрать порт к которому подключено устройство и нажать кнопку "Подключить устройство":

![Режим настройки - выбор порта](data/images//setting_mode.png)

![Режим настройки - подключение](data/images//setting_mode2.png)

### Инструкция по созданию Telegram бота

Для изменения параметров нужно получить Telegram токен и чат ID. Для этого следует нажать кнопку «Инструкция получения токена бота и ID номер чата», тогда откроется дополнительное окно с инструкцией:

![Инструкция по Telegram](data/images//tele.png)

### Настройка параметров устройства

Для изменения параметров устройства нужно нажать кнопку «Настройка параметров устройства»:

![Окно настройки параметров](data/images//setting_mode3.png)

### Результат настройки

После введения параметров они выведутся в окне отладки:

![Вывод результатов настройки](data/images//setting_mode4.png)

А устройство должно перезагрузиться с новыми параметрами:

![Перезагрузка с новыми параметрами](data/images//reboot.png)

### Рабочий режим

После загрузки и подключения к интернету устройство пришлёт сообщение о готовности к работе в Telegram, а на экране устройства высветится текущая температура и индикатор движения в виде полоски:

![Работа устройства - Telegram](data/images//work.png)

![Работа устройства - дисплей](data/images//work2.png)

*В случае если будет обнаружено движение, индикатор будет красным, если движения нет – зелёным.*

### Потеря интернет-соединения

Если подключение к интернету пропадёт – устройство выведет на дисплей предупреждение и автоматически перезагрузится через 5 секунд:

![Потеря соединения](data/images//reboot2.png)
